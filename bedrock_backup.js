...
const databaseSchemaContext = {
    tables: {
        ...
        users: {
            columns: [
                "first_name", "middle_name", "last_name", "hire_date", "relieving_date", "retirement_date", "user_no",
...
/**
 * Function to communicate with AWS Bedrock to convert NLQ into SQL
 * @param {String} naturalQuery - The natural language query input by the user
 * @returns {Promise<String>} SQL query generated by Bedrock
 */
async function getSQLFromNLQ(naturalQuery, schema) {
    // Add database context to the NLQ prompt for Titan
    const prompt = `Generate an SQL query for schema "${schema}" based on the following structure and request. 
                    The schema has the following tables and columns:

                    ${JSON.stringify(databaseSchemaContext.tables, null, 2)}

                    Generate a Postgres SQL query for the following request: "${naturalQuery}"
                    Only generate queries relevant to the provided database schema, and avoid any restricted columns.
    ...
    try {
        ...
        // Decode the Uint8Array body response and parse JSON
        // const bodyString = new TextDecoder('utf-8').decode(response.body);
        // const parsedResponse = JSON.parse(bodyString);

        // let outputText = parsedResponse.results?.[0]?.outputText || '';
        // outputText = outputText.replace("\nBot: ", "").trim();

        // return outputText; // Return the cleaned output text
        return '';
    } catch (error) {
...
/**
 * Function to validate the SQL query against the schema context
 * @param {String} sqlQuery - The SQL query to be validated
 */
function validateSQLQuery(sqlQuery) {
    ...
    const queryColumns = sqlQuery.match(/SELECT\s+([\w,\s]+)/i)?.[1].split(",") || [];
    ...
    for (const table of queryTables) {
        ...
        for (const column of queryColumns) {
            if (restrictedColumns.includes(column.trim())) {
                throw new Error(`Query references a restricted column: ${column.trim()}`);
            }
            if (!allowedColumns.includes(column.trim())) {
                throw new Error(`Query references an invalid column: ${column.trim()} in ${table}`);
            }
        }
    }
    return sqlQuery;
...
/**
 * Function to execute the SQL query using Sequelize
 * @param {String} sqlQuery - The SQL query to be executed
 * @param {String} schema - The schema where the query will be executed
